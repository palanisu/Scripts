...
hosts_file=$1
OUTPUT_REPORT="VM_DETAILS_REPORT_${DATE}.csv"
...
output='Kernel_Version|OS_Release|Latest_Version|No_of_CPU|Mem_in'
cmd_Kernel_Version='$(sudo /bin/uname -r)'
cmd_OS_Release='$(cat /etc/oracle-release)'
cmd_Latest_Version='$(rpm -qa --last | grep -w kernel-uek | grep -v firmware | sort -nr | head -1 | awk -F " " '\''{ print $1 }'\'')'


cmd_Latest_Installed_On='$(rpm -qa --last | grep -w kernel-uek | grep -v firmware | sort -nr | head -1 | awk -F " " '\''{ print $3$4$5 }'\'')'
