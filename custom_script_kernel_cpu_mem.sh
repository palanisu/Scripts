...
echo -e "${ip},${hostname},Connected,${res}" > out_${DATE}_$$_${ip}.csv
}


finalize ()
{
echo "IP,Host Name,Status${header}" > ${OUTPUT_REPORT} 
...
hosts_file=$1
OUTPUT_REPORT="/home/palani/ibm-git/outputs/Patch_CPU_Mem_REPORT_${DATE}.csv"
...
output='Cur_Kernel_Version|OS_Release|Latest_Ins_Version|No_of_CPU|Mem_in_GB'
cmd_Cur_Kernel_Version='$(sudo /bin/uname -r)'
cmd_OS_Release='$(cat /etc/oracle-release)'
cmd_Latest_Ins_Version='$(sudo rpm -qa --last | grep -w kernel-uek | grep -v firmware | sort -nr | head -1 | awk -F " " '\''{ print $1 }'\'')'
cmd_No_of_CPU='$(sudo lscpu | grep "CPU(s):          "| awk '\''{ print $2 }'\'')'
cmd_Mem_in_GB='$(free -g | grep -w "Mem:          " | awk '\''{ print $2 }'\'')'
...
while read IP host_name
       	do	
			...
			get_os_details $IP ${CUSTOMER} ${DC} ${host_name} &
                        pid="$pid $!"
