...
hosts_file=$1
OUTPUT_REPORT="/home/palani/ibm-git/outputs/OSW_REPORT_${DATE}.csv"
...
output='OSW_Enable|OSW_Running|Root_PW_Min'
cmd_OSW_Enable='$(sudo systemctl is-enabled oswatcher > /tmp/osw.txt 2>/dev/null; cat /tmp/osw.txt)'
cmd_OSW_Running='$(sudo systemctl status oswatcher |grep "Active:" |awk  '\''{ print $2$3 }'\'')'
cmd_Root_PW_Min='$(sudo chage -m 0 root; sudo chage -l root | grep "Minimum number of " | awk -F ":" '\''{ print $2 }'\'')'
